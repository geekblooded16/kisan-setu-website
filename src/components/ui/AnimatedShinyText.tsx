...
const toneClasses: Record<ShimmerTone, string> = {
  ...
  amber: 'text-amber-500/90 bg-gradient-to-r from-transparent via-amber-300/90 via-50% to-transparent',
}
  ...
  return (
    <span
      ...
      className={`${
        tone === 'light'
          ? toneClasses[tone]
          : `animate-shiny-text bg-clip-text bg-no-repeat [background-position:0_0] [background-size:var(--shiny-width)_100%] ${toneClasses[tone]}`
      } ${className}`}
    >
      {children}
    </span>
